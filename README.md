...
当前实现因此不再强制 `s50`，而是严格复刻手工成功的网页链路：每个新日期切片先用官网 URL 初始化页面条件，然后日期查询统一通过官网 `loadData1545184311000` 完成排序与分页；暂时把 `page_size` 固定为手工成功 HAR 中的 `5`。这不是认定 `pageSize=15` 一定有问题，而是先把请求形态收敛到已经被 HAR 证明成功的版本。
